% 2019_A Q3
% time: 2019/09/13 20:40
% author: null

% Task: 喷油嘴工作过度测试失败（每帧检测是否有进油行为，如果有且喷油嘴处于空闲状态 ，喷油嘴即进行工作），此机制使得 喷油嘴工作过度。
clear all;
...
i=1;
for omega=100*pi % unit is rad/s . Cam angular velocity
    a=0;
    b=0.01;
    c=10000;
    n=floor((c-a)/b + 1);
    ...
    for T_Need=15
        ...
        for t=a:b:c % unit is ms. Simulate the entire working time frame
            ...
            if k>1 
                if Rho_pump_i(k,1)>Rho_fuel_i(k-1,1) && sign == 0            
                    t_start = t; %The needle valve works from the current time t.
                    sign = 1;
                    elseif Rho_pump_i(k,1)<=Rho_fuel_i(k-1,1) && t-t_start>T_Need 
                        sign = 0;
                    elseif t_start>a
                        sign = 1;
                    else
                ...
                Needle_t=floor(100*mod(t-t_start,100)+1); %Index of time t in Needle data
