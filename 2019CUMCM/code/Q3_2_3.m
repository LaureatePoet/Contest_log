...
i=1;
for omega=50*pi % unit is rad/s . Cam angular velocity
    ...
    P_aims=100*ones(n,1);
    sign_break = 0;
    for t1=7.7 % Range: 0~100, indicating the starting working time of the injector during the period T=100ms.
        k=1;
...
%                 A_out2(k,1) = pi*((h_Needle2(k,1)+1.25/(tan(pi*9/180)))*tan(pi*9/180))^2-pi*1.25^2;
            ...
            if abs(P_tube_i(k,1)-100)< 0 % If the deviation is greater than 5, the loop is jumped out and the next traversal is performed.
                sign_break=1;
                break;
            end
            k=k+1;
